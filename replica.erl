...
next(Leaders, Slot_In, Slot_Out, Requests, Proposals, Decisions, Database) ->
  receive
    ...
    {decision, S, C} ->  % decision from commander
      ...
      Slot_Out_Ds = lists:filter(fun({Slot, _}) -> Slot == Slot_Out end, N_Decisions),
      {Slot_Out_Ps, Others} = lists:partition(fun({Slot, _}) -> Slot == Slot_Out end, Proposals),
      {N_Props, N_Requests, N_Slot_Out} = decide(S, C, Slot_Out_Ds, Slot_Out_Ps, Requests, Database, Slot_Out),
  ...
  if Len_Ds > 0 ->
    ...
    if Len_Ps > 0 ->
      ...
      if CPrimePrime /= CPrime ->
        N_Requests = [CPrimePrime] ++ Requests;
      true ->
        N_Requests = Requests
      end;
    true ->
...
perform(Database, {Client, Op, Cid}, Decisions, Slot_Out) ->
  Lower_Slot_In_Decs = lower_slot_in_decs(Slot_Out, {Client, Op, Cid}, Decisions),
  if Lower_Slot_In_Decs ->
    N_Slot_Out = Slot_Out + 1;
  true ->
    N_Slot_Out = Slot_Out
  end,
  Database ! {execute, Op},
